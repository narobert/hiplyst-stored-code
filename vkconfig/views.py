...
def add_track(request):
    ...
    if request.method == 'POST':
        ...
        track = Track.objects.create(playlist = playlist, url = track_url, aid = track_aid, title = track_title, artist = track_artist, duration = track_duration, genre = track_genre)
    return HttpResponseRedirect('/')
...
def getPlaylistTracks(request):
    ...
    playlist = Playlist.objects.get(id = playlist_id)
    playlist_tracks = Track.objects.filter(playlist = playlist)
    tracks = [pl.for_json() for pl in playlist_tracks]

    return {"status": "OK", "p_tracks": tracks}
